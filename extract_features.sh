...
### Run all modes for eval (PS3C Final Evaluation)
for extract_mode in ${extract_modes[@]}; do
    echo -e "\033[33m\nExtracting features from eval dataset with $extract_mode mode...\033[0m"
    python infer.py \
    --infer_mode extract_features \
    --extract_mode $extract_mode \
    --dataset eval \
    ...
    --model_name $model_name \
    --load_ckpt $load_ckpt \
    --batch_size 128 --num_workers 32 ### 9.3GB GPU + 118GB RAM
done
